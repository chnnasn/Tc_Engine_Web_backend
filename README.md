...
本地开发分支：`codex/aspnet-sqlite`。使用 .NET 10 和 Microsoft.Data.Sqlite，SQL 参数化，启动时在事务中按 `PRAGMA user_version` 应用 `Migrations/001_initial.sql`。SQLite 使用 WAL 和外键约束。
...
```powershell
dotnet restore server/TomCat.Api/TomCat.Api.csproj --configfile prototypes/NuGet.Config
$env:ASPNETCORE_ENVIRONMENT = 'Development'
dotnet run --project server/TomCat.Api --no-launch-profile -- --urls http://127.0.0.1:5080
```

默认数据库为 `server/TomCat.Api/App_Data/tomcat.db`。通过环境变量 `Storage__Directory` 指定持久化目录，数据库和 Cookie 加密密钥都会保存在该目录。该目录已被 Git 忽略。部署时挂载持久化卷，备份应包含数据库和密钥，运行中的 SQLite 应通过备份 API 或在服务停止后备份，不能只拷贝活跃的 `.db` 文件而漏掉 WAL。
...
保存修订契约与 `src/platform/cloud-client.ts` 一致。首次保存要求 `If-None-Match: *`，随后必须发送上次返回的 `If-Match: "revisionId"`。缺少条件返回 428，过期条件返回 412 和当前 ETag。版本插入和项目指针更新在同一个持有写锁的事务中完成，防止并发覆盖。
...
```powershell
dotnet build server/TomCat.Api -c Release --no-restore
